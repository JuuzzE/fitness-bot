...
if not TELEGRAM_TOKEN:
    ...
    # exit(1) # Раскомментировать, если нужно прерывать запуск без токена
...
# (Код функций start_command, handle_gender_and_ask_age, handle_age_and_ask_height,
# handle_height_and_ask_weight, handle_weight_and_ask_activity, handle_activity_and_ask_goal,
# process_final_profile, cancel_onboarding взят из предыдущей версии v2.3/v2.2,
# так как он был отлажен и включал логирование и исправление Markdown)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    else: # Иначе это может быть возврат на предыдущий шаг или случайный /start
...
async def process_final_profile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if ud.get(TDEE) and ud.get(TARGET_CALORIES):
            ...
            if weekly_weight_change_kg < -0.05: # Уменьшил порог для отображения
                weight_change_prediction_text = (
                    f"📈 При таком режиме ты можешь терять примерно "
                    f"*{abs(weekly_weight_change_kg):.1f} кг* в неделю.\n"
                )
            elif weekly_weight_change_kg > 0.05: # Уменьшил порог для отображения
                weight_change_prediction_text = (
                    f"📈 При таком режиме ты можешь набирать примерно "
        ...
        try:
            await query.edit_message_text("Ой, что-то пошло не так при расчете твоего профиля. 😥 Попробуй начать заново с /start.")
...
async def cancel_onboarding(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not context.user_data.get(PROFILE_COMPLETE):
        ...
        await update.message.reply_text("👍 Твой профиль уже создан. Если хочешь начать заново, используй /start (старый профиль будет сброшен).")
...
async def my_profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ud = context.user_data
    bmi_interp = get_bmi_interpretation(ud.get(BMI))
    weight_change_prediction_text = "" # Повторяем логику для отображения в профиле
    if ud.get(TDEE) and ud.get(TARGET_CALORIES):
        daily_deficit_or_surplus = ud.get(TARGET_CALORIES) - ud.get(TDEE)
        weekly_change_kcal = daily_deficit_or_surplus * 7
        weekly_weight_change_kg = weekly_change_kcal / 7700 
        if weekly_weight_change_kg < -0.05:
            weight_change_prediction_text = f"📈 Прогноз: *{abs(weekly_weight_change_kg):.1f} кг/нед.* в минус\n"
        elif weekly_weight_change_kg > 0.05:
            weight_change_prediction_text = f"📈 Прогноз: *{weekly_weight_change_kg:.1f} кг/нед.* в плюс\n"
...
async def handle_train_location_and_generate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        ...
        "Создай программу тренировок на 30-45 минут. Ответ должен быть полностью на русском языке, очень грамотным и естественным, без странных фраз или выдуманных слов. "
        "Для каждого упражнения четко укажи:\n"
        "1. Название упражнения (без использования Markdown заголовков типа `####` или `###` перед названием, просто название).\n"
        "2. Краткое и понятное описание техники выполнения этого упражнения (1-3 предложения).\n"
        "3. Количество подходов (например, 3-4).\n"
        "4. Количество повторений в каждом подходе (например, 10-15 или до около отказа для некоторых силовых).\n"
        ...
        "Раздели тренировку на секции: '## Разминка (5-7 минут)', '## Основная часть (20-30 минут)', '## Заминка (5 минут)'. Используй именно такие Markdown заголовки уровня 2. "
        ...
        "Стиль — дружелюбный и мотивирующий тренер. Ответ должен быть хорошо структурирован с использованием Markdown (списки -, жирный шрифт * для акцентов на важных моментах техники или названиях упражнений)."
    )
    ...
    reply = await ask_groq(prompt, temperature=0.6) # Немного снизил температуру для большей предсказуемости
...
async def general_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Я могу помочь с тренировками и расчетом показателей! 😊",
        parse_mode=ParseMode.MARKDOWN
    )
...
def main():
    if not TELEGRAM_TOKEN:
        logger.critical("TELEGRAM_TOKEN не найден! Бот не может запуститься.")
        return
    # GROQ_API_KEY проверяется в ask_groq
    ...
    logger.info("🤖 Бот ФитГуру v2.4 (модель Gemma, улучшенные промпты) запускается...")
